    ...
    >
      <div className="flex flex-col h-full bg-[#0e243d] border border-white/5 p-8 transition-all hover:border-sky-700/50 shadow-xl">
        ...
        <div className="flex justify-between items-start mb-4">
          ...
          {canEdit && (
            <button 
              onClick={() => router.push(`/chronicles/edit/${chronicleId}`)} 
        ...
        <div className="mt-auto pt-6 border-t border-white/5 flex gap-3">
          {fileUrl ? (
            ...
            <Link 
              href={`/chronicles/${chronicleId}`}
              className="flex-1 flex items-center justify-center py-3 border border-slate-700 hover:border-sky-700 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 hover:text-white transition-all rounded"
            >
              Ver Detalle
            </Link>
          )}
          ...
          <Link 
            href={`/chronicles/${chronicleId}`}
            className="px-4 py-3 flex items-center border border-white/5 text-slate-500 hover:text-white transition-all rounded"
          >
